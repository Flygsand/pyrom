...
from pyrom.auxiliaries import (
    sub as _sub, unpack as _unpack,
    unispace as _unispace, istrip as _istrip
)
...
def parse(path):
    with open(path) as f:
        ...
        if not _valid_rom_info(data):
            ...
            if not _valid_rom_info(data):
                raise ValueError('Not a Genesis ROM')

        name = _unispace(_sub(data, 0x150, 48))
        version = _istrip(_sub(data, 0x182, 12))
